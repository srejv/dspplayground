                  ...
                  )
{
    ...
    static int tablep = 0;
    unsigned long i;
    //tabinbuf(osc, 1, out, unsigned long framesPerBuffer,
      //        2, const double* amp,const double* freq)
    for( i=0; i < framesPerBuffer; i++ ) {
        float sine = sin(phase);
        *out++ = table->table[tablep];
        *out++ = table->table[tablep];

        if(++tablep >= table->length) { tablep = 0; }
...
void init_stuff()
{
    ...
    table = new_square(1024, 3);
    osc = new_oscilt(SAMPLING_RATE, table, 0.0);
